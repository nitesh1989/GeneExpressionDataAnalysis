...
```{r installBioconductorPackages,echo=FALSE}
...
```{r checkDataProperties,eval=TRUE}
# Dimensions
dim(dat)
```
...
```{r checkPhenotypeDataProperties,fig.height=5,fig.width=8}
str(pheno)
...
```{r checkIfSameGenesWereIdentified}
all(tG == tG2)
...
```{r checkObjectsAfterAddingAnnotation,eval=FALSE}
tG2ann
...
```{r saveResultsForLaterUse, eval = TRUE}
